...
const source = readFileSync(join(root, 'skills/exportdou/SKILL.md'), 'utf8');
const match = source.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/);
if (!match || !/^name: exportdou$/m.test(match[1])) {
  throw new Error('Expected the canonical exportdou skill and its YAML frontmatter');
}
...
// WorkBuddy locale fields belong only in this generated distribution package.
// Reuse the canonical execution instructions verbatim.
const fields = {
  name: 'exportdou',
  ...listing,
...
const output = join(root, 'dist/workbuddy/exportdou');
mkdirSync(output, { recursive: true });
writeFileSync(join(output, 'SKILL.md'), `---\n${frontmatter}\n---\n${match[2]}`);
const archive = join(root, `dist/workbuddy/exportdou-workbuddy-${version}.zip`);
